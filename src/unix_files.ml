...
let create ~id ~root =
  return (object
    method read filename =
      prerr_endline ("read: " ^ filename);
      let fullname = sprintf "%s/%s" root filename in
      (* Open the FD using the manager bindings *)
      try_lwt Lwt_unix.openfile fullname [Lwt_unix.O_RDONLY] 0
        >>= fun fd ->
        ...
        return (Some (Lwt_stream.from (fun () ->
          ...
          match len with
          | 0 -> Lwt_unix.close fd >>= fun () -> return None
    ...
    method iter_s fn =
      try_lwt
        ...
        >>= fun dir -> begin
          ...
          try_lwt
            loop ()
          finally (Lwt_unix.closedir dir)
        end
     with _ -> fail (Failure "opendir")

    method size filename =
      let fullname = sprintf "%s/%s" root filename in
      Lwt_unix.stat fullname
      >>= fun st -> return (Some (Int64.of_int st.Lwt_unix.st_size))
  end)
