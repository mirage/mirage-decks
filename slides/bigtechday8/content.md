...
[http://decks.openmirage.org/bigtechday8/](http://decks.openmirage.org/bigtechday8/)
...
- The preceding picture showed them hosted in a **2.5MB Xen unikernel** written
  in statically type-safe OCaml, including device drivers and network stack
...
> 1. __Why Unikernels?__

> 2. What are Unikernels?

> 3. How can I use Unikernels?
...
Applications and frameworks are **deeply intertwined** with system APIs, and so
lack portability
...
+ Ad hoc platform and application configuration: `/etc/` to firewalls
...
> __Unikernels__: specialised virtual machine images built from a modular
> stack adding system libraries and configuration to application code
...
+ __Portable__, to execution environments
  + E.g., Xen, POSIX, browser, and more coming
...
In OCaml, modules can be parameterised by other modules:

+ Such modules are called
  [__functors__](https://realworldocaml.org/v1/en/html/functors.html)
  + __Not__ the same as Haskell functors though!
+ Think of them as a type-safe version of C++ templates
...
[V1.mli](https://github.com/mirage/mirage/blob/master/types/V1.mli)
...
- A collection of libraries where the functors are applied, suitable for
  interactive use
...
> Build the desired unikernel by writing OCaml defining the exact configuration
> as its output
...
_Cf __Mirage__ appliance, with and without simple memoisation: this
__algorithmic__ optimisation added just 6 lines of code_
...
> 3. __How can I use Unikernels?__
...
+ __Removal of `libc`__: reduces attack surface and image size
  + Had to add floating point formatting routines back, copied from `musl`
    `libc`
+ __No hardware emulation__: Xen PV driver model only
  + ARM does not need all the legacy support of Xen/x86
+ __Deserialising boot__: device attachment, boot transactions
  + New XenStore database reduces spurious boot conflicts
  + _dom0_ device hotplug scripts run in parallel with the domain builder
...
+ Begun in January 2014 by David Kaloper and Hannes Mehnert
+ Full re-implementation of TLS including X509, ASN.1 baggage
+ Functional style means the *same* code used for trace checking
...
- **Choices**: [Mirage OS](http://openmirage.org) in OCaml,
  ...
  ClickOS for routing
- **Scenarios**: Static websites, dynamic content, custom routers. See Adam Wick
  at [QConSF](https://www.youtube.com/watch?v=oHcHTFleNtg) explain when *not* to
  use it.
- **Performance**: There's no hiding behind abstractions &mdash; HalVM *vs*
  Mirage is a fun contest in evaluating abstraction cost

> Need contributors in building a sound basis of safe protocol implementations,
> integrating verification and automated testing
...
<p style="font-size: 40px; font-weight: bold;
  ...
  <br />
  These slides published at [decks.openmirage.org](http://decks.openmirage.org)
  <br />
